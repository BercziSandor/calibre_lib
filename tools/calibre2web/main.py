...
from calibre_tools.Library import Library
...
def get_available_libraries(
        libraries_root: Path = get_script_directory().parent.parent / 'libs'):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generating static Calibre catalog for publishing on the internet.')
    parser.add_argument('--library',
                        help='The path of the library to be processed. It can be absolute or relative '
    ...
    if args.library is None:
        ...
        print(f"Finding library '{args.library}'")
        library = pathlib.Path(args.library)
        print(f"Trying as {library.absolute()}.", end='')
        if library.exists():
            print(" - found.")
        else:
            print(" - not found.")
    ...
    lib.md2html(delete_markdown=False)
